    ...
    def execute_cb(self, goal_handle):
        ...
        while rclpy.ok():
            ...
            # ------------------------------------------------------------
                ...
                if not self.snapshot_valid:
                    ...
                    self.get_logger().warn(
                        ...
                        f"(goal=({self.snap_x_goal:.3f},{self.snap_y_goal:.3f}) "
            ...
            # ------------------------------------------------------------
                ...
                # ---------------------------------------------------------
                # [UPDATED] VERIFY_POSE는 "yaw 검증"을 vision pose_yaw_err가 아니라
                #          snapshot(odom/imu) 기반 yaw_err로 수행한다.
                # - 이유: pose_yaw_err가 ±pi 근처에서 튀면(랩/뒤집힘) drift가 무한 반복될 수 있음
                # ---------------------------------------------------------

                # 1) odom/snapshot 준비 체크
                if (not self.snapshot_valid) or (not self.odom_is_fresh()) or (self.odom_yaw is None):
                    ...
                    # 2) yaw 검증은 odom(또는 fused yaw) 기반
                    yaw_now = float(self.get_fused_yaw())
                    yaw_err = normalize_angle(self.snap_yaw_goal - yaw_now)

                    # 3) vision은 "센터링(화면 중앙)" 확인에만 사용
                    marker_ok_for_center = (
                        self.marker_valid and (self.marker_id in target_ids) and self.marker_is_fresh()
                        and (self.center_x_err is not None)
                    )

                    if not marker_ok_for_center:
                        # ---------------------------------------------------------
                        # [UPDATED] VERIFY_POSE "짧은 REACQUIRE"
                        # - 마커가 잠깐 끊겨도 즉시 SEARCH로 빠지지 않음
                        # ---------------------------------------------------------
                        ...
                        # 마커 재획득 성공 -> timer reset
                        self.reacquire_started = None

                        cx = float(self.center_x_err)

                        # 4) 센터 드리프트가 크면 CENTERING으로 복귀 (기존 로직 유지)
                        ...
                        # 5) yaw 드리프트가 크면 ROTATE로 복귀 (✅ odom/snapshot 기준)
                        #
                        # - 여기서는 "pose_enter_th"를 재사용하지 않고,
                        #   ROTATE의 종료 기준(rotate_done_th)을 그대로 VERIFY의 yaw 기준으로 재사용합니다.
                        #   (전역 파라미터 추가 없이, 기존 방식 유지)
                        ...
                        # 6) 안정 홀드 후 APPROACH로 진행 (기존 로직 유지)
                        else:
                            ...
                            if (now - self.verify_started) >= self.verify_hold_sec:
                                self.get_logger().info(
                                    f"[FSM] VERIFY_POSE -> APPROACH (stable) (yaw_err={yaw_err:.3f}, cx={cx:.3f})"
                                )
                ...
                # -------------------------------------------------
                # APPROACH는 odom 기반 제어 + vision은 보정용
                # -------------------------------------------------

                # 1️⃣ snapshot / odom 유효성 체크
                ...
                or (self.odom_yaw is None):
                    ...
                    # -------------------------------------------------
                    # 5️⃣ 너무 많이 틀어지면 CENTERING 복귀
                    # -------------------------------------------------
                    if abs(head_err) > 0.6:  # 약 35도 이상
                        self.get_logger().warn("[FSM] APPROACH heading drift -> CENTERING")
                        self.state = DockState.CENTERING
                        self.reset_controllers()

                    # -------------------------------------------------
                    # 6️⃣ FINAL 구간 진입 (odom 기준)
                    # -------------------------------------------------
                    elif dist_goal <= self.final_zone_dist:
                        self.get_logger().info("[FSM] APPROACH -> FINAL_ALIGN (odom-based)")
