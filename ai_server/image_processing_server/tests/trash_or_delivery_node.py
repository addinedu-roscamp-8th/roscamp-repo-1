...
def _get_ctx(self) -> tuple[MotionCfg, GripperCfg]:
    m = MotionCfg(
...
class BTrayOrchestratorNode(Node):
    ...
    async def _run_transport_verify(self, job_id: str) -> None:
        m, g = _get_ctx(self)
        poses = _get_poses(self, "pick_tray", "base", "place_verify", "place_verify_s2")
        ...
        pick_tray_down = _pose_down(pick_tray, dz=(m.z_move))
    ...
    async def _run_handoff_pinky(self, job_id: str) -> None:
        m, g = _get_ctx(self)
        poses = _get_poses(self, "base", "place_pinky", "place_verify")

        base = poses["base"]
        place_pinky = poses["place_pinky"]
        place_verify = poses["place_verify"]

        place_pinky_down = _pose_down(place_pinky, dz=(m.z_move - m.z_offset))
        ...
        await self._go(place_pinky_down, m.speed_z, m.mode)
        await asyncio.sleep(m.settle)
    ...
    async def _run_discard(self, job_id: str) -> None:
        m, g = _get_ctx(self)
        poses = _get_poses(self, "base", "trash", "place_verify")

        base = poses["base"]
        trash = poses["trash"]
        place_verify = poses["place_verify"]

        trash_down = _pose_down(trash, dz=(m.z_move - m.z_offset))
        ...
        await self._go(trash_down, m.speed_z, m.mode)
        await asyncio.sleep(m.settle)
