...
class PrecisionDockingServer(Node):
    ...
    def marker_cb(self, msg: Marker2D):
        ...
        if msg.valid:
            ...
            # [성능 주의] INFO 로그가 20~30Hz로 나오면 성능을 저하시킬 수 있는 로그 코드이므로 주석을 권합니다.
    ...
    def create_snapshot_goal(self):
        ...
        theta = float(self.pose_yaw_err)

        # 마커 법선 단위벡터(카메라 x-z 평면): (nx, nz)
        nx = math.sin(theta)
        nz = math.cos(theta)

        # -----------------------------------------------------------------
        # [UPDATED] standoff 목표점 계산 (오버슈팅 방지 핵심)
        #
        # 요구사항(확정):
        #   - Standoff = “마커의 법선(normal) 방향 라인 위에서, 마커로부터 15cm 떨어진 지점”
        #   - 로봇은 그 지점에 서서(위치), 이후 ROTATE로 마커를 '정면'으로 바라본다(방향).
        #
        # 문제 원인(로그 기반):
        #   - pose_yaw_err(theta)가 '법선 각도'일 때, 법선 벡터의 부호(+/-)가
        #     '카메라 쪽을 향하는 법선'인지 / '카메라 반대(마커 뒤) 쪽 법선'인지가
        #     프레임 정의에 따라 달라질 수 있다.
        #   - 기존 구현이 p_marker - standoff*n 만 사용하면, nz<0 인 케이스에서
        #     tz가 증가(=마커를 지나쳐 더 전진)하는 오버슈팅이 발생할 수 있다.
        #
        # 해결 전략:
        #   - 두 후보점을 모두 계산(A/B)하고, '카메라(로봇) 쪽에 더 가까운' 후보를 선택한다.
        #     A = p_marker - standoff*n
        #     B = p_marker + standoff*n
        #   - 일반적으로 tz>0(카메라 전방)이므로 tz가 더 작은 후보가 '덜 전진'하는 standoff가 된다.
        # -----------------------------------------------------------------
        ...
        # 후보 A/B (camera frame)
        ax = tx_c - self.standoff_m * nx
        az = tz_c - self.standoff_m * nz
        bx = tx_c + self.standoff_m * nx
        bz = tz_c + self.standoff_m * nz

        # [UPDATED] 선택 규칙:
        # - tz_c > 0: tz가 더 작은 후보(카메라에 더 가까움)를 선택
        # - 그 외: 원점(카메라)까지 거리가 더 작은 후보를 선택
        if tz_c > 0.0:
            if az <= bz:
                dx_base = ax
                dz_base = az
                standoff_sign = -1  # A = marker - standoff*n
            else:
                dx_base = bx
                dz_base = bz
                standoff_sign = +1  # B = marker + standoff*n
        else:
            da = math.hypot(ax, az)
            db = math.hypot(bx, bz)
            if da <= db:
        ...
        # [UPDATED] dx_base/dz_base는 '선택된 standoff 점의 camera(x,z)' 좌표이다.

        # base frame (x: forward, y: left)로 변환
        # 여기서는 카메라 x가 좌/우, z가 전방이므로:
        # - dz_base -> base forward (x)
        # - dx_base -> base left(y)  (tx 양수면 "오른쪽"일 수도 있는데 실제 부호는 현장 기준)
        # 기존 로그에서 tx가 음수일 때 왼쪽에 있다고 보는 흐름이 있으므로,
        # base_y를 tx 그대로 두고, 필요 시 부호 튜닝 포인트로 둔다.
        base_x = dz_base
        base_y = dx_base

        # odom으로 회전시켜 목표점 생성
        # odom_x = x_now + cos(yaw)*base_x - sin(yaw)*base_y
        # odom_y = y_now + sin(yaw)*base_x + cos(yaw)*base_y
        x_goal = x_now + math.cos(yaw_now) * base_x - math.sin(yaw_now) * base_y
        y_goal = y_now + math.sin(yaw_now) * base_x + math.cos(yaw_now) * base_y

        # [UPDATED] 목표 yaw:
        # "마커 법선을 정면으로 바라보는 yaw"가 필요하므로,
        # 현재는 pose_yaw_err가 camera frame에서의 법선 yaw이므로
        # 목표 yaw를 단순히 yaw_now + (법선 yaw를 줄이는 방향)으로 두지 않고,
        # ROTATE 단계에서 '마커 재획득 후' pose_yaw_err로 미세정렬 할 수도 있음.
        #
        # 이번 Blind ROTATE의 1차 목표는:
        # - 스냅샷 순간의 yaw_rad(마커 중심 방향)를 0으로 만드는 방향으로 회전해 "정면 재획득"을 유도
        # - yaw_rad는 base 기준 좌우각 (atan2(tx, tz)) 성격
        #
        # 따라서 yaw_goal = yaw_now + yaw_rad_snapshot 보정(부호는 기존 제어와 일관)
        ...
        self.get_logger().info(
            ...
            f"(tx={tx:.3f}, tz={tz:.3f}, pose_yaw_err={theta:.3f}, yaw_rad_snap={yaw_rad_snap:.3f}, standoff_sign={standoff_sign:+d}, standoff_cam=({dx_base:.3f},{dz_base:.3f}))"
        )
        # [성능 주의] INFO 로그가 잦으면 성능을 저하시킬 수 있는 로그 코드이므로 주석을 권합니다.
        ...
        while rclpy.ok():
                    ...
                    if not marker_ok:
                        ...
                        if condition_met:
                            ...
                            if not self.snapshot_valid:
                                ...
                                self.get_logger().info("[FSM] CENTERING -> FACE_ALIGN_TRANSLATE (center ok, snapshot ready)")
            ...
            # ------------------------------------------------------------
                ...
                if not self.snapshot_valid:
                    ...
                    if dist_goal <= self.translate_done_th:
                        self.get_logger().info("[FSM] TRANSLATE -> FACE_ALIGN_ROTATE (dist_goal reached)")
            ...
            # ------------------------------------------------------------
                ...
                if not self.snapshot_valid:
                    ...
                    v_cmd = 0.0
                    w_cmd = clamp(self.rotate_w_kp * yaw_err, -self.rotate_max_w, self.rotate_max_w)
                    ...
                    if abs(yaw_err) <= self.rotate_done_th:
                        # 다음 단계는 "마커 재획득"이 필요할 수 있음.
                        # 가장 단순/안전: SEARCH로 돌아가 마커를 다시 찾고 이후 VERIFY로
                        self.get_logger().info("[FSM] ROTATE -> SEARCH (yaw aligned, reacquire marker)")
                        self.state = DockState.SEARCH
                        self.reset_controllers()
