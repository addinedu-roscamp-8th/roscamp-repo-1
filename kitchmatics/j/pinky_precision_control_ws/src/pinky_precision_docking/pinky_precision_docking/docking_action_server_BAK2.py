...
class PrecisionDockingServer(Node):
    ...
    def __init__(self):
        ...
        # [UPDATED-FIX] 시간 기준 분리
        self.last_msg_time = 0.0     # 어떤 msg든 수신한 시각 (valid=False 포함)
        self.last_valid_time = 0.0   # valid=True 수신 시각
        ...
        # [UPDATED-FIX] timeout의 의미를 명확히: "스트림/유효" 분리 운영
        self.marker_timeout_sec = 0.6        # 스트림이 끊겼는지 판단(약간 여유)
        self.valid_marker_timeout_sec = 0.8  # valid가 마지막으로 들어온지(조금 더 여유)
        ...
        # [UPDATED-FIX] grace를 marker 유실 상황에서 조금 더 버티게 (실험값)
        self.align_loss_grace = 0.50  # 0.30 -> 0.50 (reacquire 여유)
    ...
    def marker_cb(self, msg: Marker2D):
        # [UPDATED-FIX] valid=False 메시지도 "스트림 살아있음"으로 간주하여 수신 시간을 갱신
        ...
        if msg.valid:
            # [UPDATED-FIX] valid 수신 시간을 별도로 기록
            self.last_valid_time = self.last_msg_time

            # 로그 과다로 콜백이 밀릴 수 있으므로, INFO는 유지하되 필요 시 추후 throttle 권장
    ...
    def execute_cb(self, goal_handle):
        ...
        while rclpy.ok():
            ...
            # [UPDATED-FIX] "스트림"이 끊긴 경우만 watchdog로 강하게 대응
            ...
            if self.state == DockState.SEARCH:
                ...
                # [UPDATED-FIX] SEARCH 진입 조건은 "valid가 최근에 들어왔는지"로 판단
            ...
            # =========================
                ...
                if not timeout_occurred:
                    # [UPDATED-FIX] marker_ok는 "valid가 최근에 들어왔는지" 기준
                    marker_ok = (
